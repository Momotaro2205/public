...
class User
{
  ...
  static function authUser($email, $pass)
  {
    ...
    $authResult = $mysqli->query("SELECT * FROM `users` WHERE `email`='$email'");
    $authResult = $authResult->fetch_assoc();

    if (password_verify($pass, $authResult["pass"])) {
      return json_encode(["authResult" => "authorized"]);
      ...
      return json_encode(["authResult" => "access danied"]);
    }
  }
}
